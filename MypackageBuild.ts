...
fs.copySync(path.normalize(`${__dirname}/${modName}`), path.normalize(`${pathToMods}/${packageNameRes}`));
console.log("Build files copied.");

fs.copySync(path.normalize(`${__dirname}/${modName}`), path.normalize(`${__dirname}/dist/user/mods/${packageNameRes}`));
