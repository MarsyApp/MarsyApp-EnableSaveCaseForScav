...
class Mod {
    ...
    hookRoutes() {
        this.router.registerDynamicRouter("GenerateKeyInBots", [
            {
                url: "/GenerateKeyInBots/GetData",
                action: (url, info, sessionId, output) => {
                    return this.getData(url, info, sessionId, output);
                }
            },
            {
                url: "/GenerateKeyInBots/GetInfo",
                action: (url, info, sessionId, output) => {
    ...
    getModInfo(url, info, sessionId, output) {
        ...
        modOutput.data = { ...{ path: this.path.resolve(this.modLoader.getModPath("MarsyApp-GenerateKeyInBots")) } };
        modOutput.status = 0;
    ...
    getData(url, info, sessionId, output) {
        ...
        if (!profile.GenerateKeyInBotsItems) {
            profile.GenerateKeyInBotsItems = [];
        }
        modOutput.data = { GenerateKeyInBotsItems: profile.GenerateKeyInBotsItems, GenerateKeyInBotsItemsIgnore: config.ignoreList || [] };
        modOutput.status = 0;
        return this.jsonUtil.serialize(modOutput);
    }
    ...
    getTraderAssort(sessionId, traderId, oldGetTraderAssort) {
        ...
        if (needDelete) {
            assort.items = assort.items.filter((item) => {
                return profile.GenerateKeyInBotsItems.includes(item._tpl) || config.ignoreList.includes(item._tpl);
            });
        }
        else {
            assort.items.map((item) => {
                if (!profile.GenerateKeyInBotsItems.includes(item._tpl)) {
                    item.upd = {
    ...
    getOffersForSearchType(offers, sessionID) {
        ...
        offers = offers.filter((offer) => {
            return offer.items.every((item) => {
                /*if(!(profile.GenerateKeyInBotsItems.includes(item._tpl) || config.ignoreList.includes(item._tpl))) {
                }*/
                return profile.GenerateKeyInBotsItems.includes(item._tpl) || config.ignoreList.includes(item._tpl);
            });
        });
    ...
    unlockItems(items, sessionID) {
        ...
        const origCount = profile.GenerateKeyInBotsItems.length;
        for (const item of items) {
            if (!item._tpl || profile.GenerateKeyInBotsItems.includes(item._tpl))
                continue;
            profile.GenerateKeyInBotsItems.push(item._tpl);
            logger.info(`[GenerateKeyInBotsItems] Unlocked ${item._tpl} for player ${profile.info?.username}`);
        }
        logger.info(`[GenerateKeyInBotsItems] Unlocked ${profile.GenerateKeyInBotsItems.length - origCount} items for player ${profile.info?.username}`);
    }
}
